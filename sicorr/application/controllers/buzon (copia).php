	...
	class Buzon_controller extends Controller {
		...
		private function _pre_inbox( )
		{
         if (!isset($_SESSION['user_id'])) url::redirect('login/index');
			$i = 0;
			$buzon = array( ); 
			$corr = ORM::factory('correspondencia')->where(array('inactivo'=>0))->find_all( );
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			foreach($corr as $c)
			{
				$buzon[$i] = array(
            ...
            /*
         	$corrinstruccion = ORM::factory('corrinstruccion')->where(array('correspondencia_id'=>$c->id))->find_all( );
				foreach ($corrinstruccion as $ci)
				{
					$buzon[$i]['instrucciones'][$ci->instruccion->descripcion]['dependencias'][ ] = $ci->dependencia_id;
					$buzon[$i]['instrucciones_keys'][ ] = $ci->instruccion->descripcion;
				}
            */
		...
		public function index( )
		{
         ...
         $this->wrapper->js = array('buzon','lightbox/js/lightbox');
         $this->wrapper->estilos = array('lightbox');
